...
public:
    ...
    template<typename T>
    std::vector<T> solve(std::vector<T> rhs, const std::vector<T> &fixedVarValues = std::vector<T>()) const {
        const size_t n = rows();
        if (rhs.size() != n) throw std::runtime_error("Incorrect RHS size");
        factorize();

        const auto &iden = m_factorization->inv_den;
        const auto &cp   = m_factorization->cPrime;

        auto getFixedVarValue = [&](size_t i) {
            if (fixedVarValues.size() == 0) throw std::runtime_error("Fixed var values were not passed");
            if (fixedVarValues.size() <= i) throw std::runtime_error("Not enough fixed var values were passed");
            return fixedVarValues[i];
        };

        // Forward-solve
        if (m_isFixed[0]) rhs[0] = getFixedVarValue(0);
        else              rhs[0] *= iden[0];
        for (size_t i = 1; i < n; ++i) {
            if (m_isFixed[i]) rhs[i] = getFixedVarValue(i);
